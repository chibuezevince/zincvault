                                        ...
                                        <div class="row">
                                            <div class="col d-flex justify-content-between">
                                                ...
                                                <div>
                                                    ...
                                                    <!-- Subtitle -->
                                                    <h2 class="mb-0">
                                                        ${{number_format(user_details()->dollar_balance, 2)}}
                                                    </h2>
        
                                                    <!-- Comment -->
                                                    <p class="fs-6 text-muted mb-0">
                                                        @if (transaction_log('Debit', 'dollar'))
                                                           <span class="text-danger"> - ${{number_format(transaction_log('Debit', 'dollar')->amount, 2)}}</span>&nbsp;
                                                        @endif    
                                                        @if(transaction_log('Credit', 'dollar')) 
                                                       <span class="text-success"> + ${{number_format(transaction_log('Credit', 'dollar')->amount, 2)}}</span>
                                                        @endif  
                                                    </p>
                                                </div>
                                        ...
                                        <div class="row">
                                            <div class="col d-flex justify-content-between">
                                                ...
                                                <div>
                                                    ...
                                                    <!-- Subtitle -->
                                                    <h2 class="mb-0">
                                                        €{{number_format(user_details()->euro_balance, 2)}}
                                                    </h2>
        
                                                    <!-- Comment -->
                                                    <p class="fs-6 text-muted mb-0">
                                                            @if (transaction_log('Debit', 'euro'))
                                                               <span class="text-danger"> - €{{number_format(transaction_log('Debit', 'euro')->amount, 2)}}</span>&nbsp;
                                                            @endif    
                                                            @if(transaction_log('Credit', 'euro')) 
                                                           <span class="text-success"> + €{{number_format(transaction_log('Credit', 'euro')->amount, 2)}}</span>
                                                            @endif      
                                                        </p>
                                                </div>
                            ...
                            <div class="table-responsive">
                                <table class="table align-middle table-edge table-hover table-nowrap mb-0">
                                    ...
                                    <tbody class="list">
                                        ...
                                        @else
                                        @foreach (transactionLogs(user()->username) as $log)
                                        <tr>
                                            ...
                                            <td>{{$log->currency}}</td>
                                            <td class="date"><span class=""></span>{{$log->time}}</td>
                                            <td>{{currencyLogo($log->currency)}}{{$log->amount}}</td>
                                            <td class="date"><span class="badge @if($log->status==1)text-bg-success @elseif($log->status==2)text-bg-info @else text-bg-danger @endif">{{status($log->status)}}</span></td>
                                        </tr>
                                        @endforeach
                                        @endif
                                    </tbody>
